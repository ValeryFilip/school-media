...
6. **WebM** в `stepik/gifs/` — пережать с меньшим битрейтом (например ffmpeg `-b:v 800k` или онлайн-сервис).
7. **technical/404.png** — WebP или сжатый PNG.
8. **Lighthouse: «Размер изображения превышает размер контейнера»** — иконки из `technical/` (telegram, vk, whatsapp, youtube и др.) отображаются в блоках ~20–40px, но файлы могут быть 512×512. Чтобы убрать предупреждение: сохранить эти иконки в размере **48×48** или **64×64** px и заменить файлы в `public/images/technical/`. В коде у всех таких `<img>` уже проставлены `width` и `height` по размеру отображения.
